...
echo  "$($PSQL "SELECT DISTINCT(teams.name) FROM games INNER JOIN teams ON games.winner_id = teams.team_id OR games.opponent_id = teams.team_id WHERE games.year = 2014 AND games.round = 'Eighth-Final' ORDER BY teams.name;")"
...
echo  "$($PSQL "SELECT games.year, teams.name FROM games INNER JOIN teams ON games.winner_id = teams.
team_id WHERE round = 'Final' ORDER BY games.year;")"
